...
public class MongoDbOutput extends BaseStep implements StepInterface {
	...
	/**
	 * Holds an original batch of rows (corresponding to the converted
	 * documents)
	 */
	...
	public MongoDbOutput(StepMeta stepMeta,
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi)
		...
		if (row == null) {
			...
			List<MongoDbOutputMeta.MongoIndex> indexes = m_meta
					.getMongoIndexes();
		...
		if (first) {
			...
			String batchInsert = environmentSubstitute(m_meta
					.getBatchInsertSize());
			...
			List<MongoDbOutputMeta.MongoField> mongoFields = m_meta
					.getMongoFields();
			...
			if (m_meta.getUpdate()) {
				...
				if (updateQuery != null) {
					...
					if (!m_meta.getModifierUpdate()) {
						...
						if (log.isDebug()) {
							logDebug("Insert Update mongodb data: ");
							logDebug(BaseMessages
									.getString(
											PKG,
				...
				DBObject mongoInsert = MongoDbOutputData.kettleRowToMongo(
						m_data.m_userFields, getInputRowMeta(), row,
						MongoDbOutput.this, m_mongoTopLevelStructure,
						m_data.m_hasTopLevelJSONDocInsert, false);
	...
	protected void commitUpdate(DBObject updateQuery, DBObject insertUpdate,
		...
		while (retrys <= m_writeRetries && !isStopped()) {
			...
			try {
				// TODO It seems that doing an update() via a secondary node
				// does not
				...
				// a MongoException if you are not talking to the primary. So we
				// need
				// some logic to check whether or not the connection
				// configuration
				// contains the primary in the replica set and give feedback if
				// it
				// doesnt
				result = m_data.getCollection().update(updateQuery,
						insertUpdate, m_meta.getUpsert(), m_meta.getMulti());
		...
		if ((retrys > m_writeRetries || isStopped()) && lastEx != null) {
			...
			if (getStepMeta().isDoingErrorHandling()) {
				putError(getInputRowMeta(), row, 1, lastEx.getMessage(), "",
						"MongoDbOutput");
			} else {
	...
	protected CommandResult batchRetryUsingSave(boolean lastRetry)
		...
		int count = 0;
		logBasic(BaseMessages.getString(PKG,
				"MongoDbOutput.Messages.CurrentBatchSize", m_batch.size()));
			...
			try {
				...
				if (!lastRetry) {
					logBasic(BaseMessages
							.getString(
									PKG,
									"MongoDbOutput.Messages.SuccessfullySavedXDocuments",
									count));
				...
				// Send this one to the error stream if doing error handling
				if (getStepMeta().isDoingErrorHandling()) {
					putError(getInputRowMeta(), correspondingRow, 1,
							ex.getMessage(), "", "MongoDbOutput");
				} else {
		...
		logBasic(BaseMessages.getString(PKG,
				"MongoDbOutput.Messages.SuccessfullySavedXDocuments", count));
	...
	protected void doBatch() throws KettleException {
		...
		while (retries <= m_writeRetries && !isStopped()) {
			...
			try {
				...
				// avoid exception if a timeout issue occurred and it was
				// exactly the first attempt
				boolean shouldNotBeAvoided = !isTimeoutException(me)
						&& (retries == 0);
				...
				if (retries <= m_writeRetries) {
					if (shouldNotBeAvoided) {
						// skip logging error
						// however do not skip saving elements separately during
						// next attempt to prevent losing data
						logError(BaseMessages
								.getString(
										PKG,
										"MongoDbOutput.Messages.Error.ErrorWritingToMongo", //$NON-NLS-1$
	...
	private static boolean isTimeoutException(MongoException me) {
		return (me instanceof MongoException.Network)
				&& (me.getCause() instanceof SocketTimeoutException);
	}
		...
		if (super.init(stepMetaInterface, stepDataInterface)) {
			...
			if (!Const.isEmpty(m_meta.getWriteRetries())) {
				try {
					...
					m_writeRetries = MongoDbOutputMeta.RETRIES;
				}
			}

			if (!Const.isEmpty(m_meta.getWriteRetryDelay())) {
				try {
					m_writeRetryDelay = Integer.parseInt(m_meta
							.getWriteRetryDelay());
				} catch (NumberFormatException ex) {
			...
			int port = Const.toInt(environmentSubstitute(m_meta.getPort()),
					27017);
			...
			try {
				...
				m_data.setConnection(MongoClientWrapperFactory
						.createMongoClientWrapper(m_meta, this, log)); // MongoDbOutputData.connect(
																		// m_meta,
																		// this,
																		// log )
																		// );
				...
				m_data.setCollection(m_data.getConnection().getCollection(db,
						collection));

				try {
					m_mongoTopLevelStructure = MongoDbOutputData
							.checkTopLevelConsistency(m_meta.m_mongoFields,
									MongoDbOutput.this);
	...
	final void checkInputFieldsMatch(RowMetaInterface rmi,
		...
		for (MongoDbOutputMeta.MongoField field : mongoFields) {
			...
			logBasic("mongo filed name: " + mongoMatch + " <END>");	
		}
		for (int i = 0; i < rmi.size(); i++) {
			String metaFieldName = rmi.getValueMeta(i).getName();
			//metaFieldName = metaFieldName.replaceAll("[\t\n\r]", "");
			actual.add(metaFieldName);
			logBasic("meta filed name: " + metaFieldName + " <END>");
		}
		...
		if (!actual.isEmpty()) {
			...
			logBasic(BaseMessages.getString(PKG,
					"MongoDbOutput.Messages.FieldsNotToBeInserted",
					b.toString()));
		}
	}
}
